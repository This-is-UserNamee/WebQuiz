...
const handlePlayerDisconnect = (socketId) => {
  ...
  if (roomId) {
    handlePlayerLeave({ id: socketId }, roomId, true);
  }
...
/** プレイヤーの退出・切断を処理する */
const handlePlayerLeave = (socket, roomId, isDisconnect = false) => {
  const room = state.rooms[roomId];
  if (!room || !room.players[socket.id]) return;

  const playerName = state.players[socket.id]?.name || '不明なプレイヤー';
...
const startQuestionLifecycle = (roomId) => {
  const room = state.rooms[roomId];
  if (!room) return;
...
const startTimer = (roomId, duration) => {
  const room = state.rooms[roomId];
  if (!room) return;
